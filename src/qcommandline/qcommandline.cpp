...
const QCommandLineConfigEntry QCommandLine::helpEntry = { QCommandLine::Switch, QLatin1Char('h'), QLatin1String("help"),
    tr("Display this help and exit"), QCommandLine::Optional };

const QCommandLineConfigEntry QCommandLine::versionEntry = { QCommandLine::Switch, QLatin1Char('V'),
    QLatin1String("version"), tr("Display version and exit"), QCommandLine::Optional };

QCommandLine::QCommandLine(QObject* parent)
    : QObject(parent)
    , d(new QCommandLinePrivate) {
    setArguments(QCoreApplication::instance()->arguments());
}

QCommandLine::QCommandLine(const QCoreApplication& app, const QCommandLineConfig& config, QObject* parent)
    : QObject(parent)
    , d(new QCommandLinePrivate) {
...
QCommandLine::QCommandLine(int argc, char* argv[], const QCommandLineConfig& config, QObject* parent)
    : QObject(parent)
    , d(new QCommandLinePrivate) {
...
QCommandLine::QCommandLine(const QStringList args, const QCommandLineConfig& config, QObject* parent)
    : QObject(parent)
    , d(new QCommandLinePrivate) {
    setArguments(args);
    setConfig(config);
...
QCommandLine::~QCommandLine() { delete d; }

void QCommandLine::setConfig(const QCommandLineConfig& config) { d->config = config; }
...
QCommandLineConfig QCommandLine::config() { return d->config; }
...
void QCommandLine::setArguments(QStringList args) { d->args = args; }

QStringList QCommandLine::arguments() { return d->args; }

void QCommandLine::enableHelp(bool enable) { d->help = enable; }

bool QCommandLine::helpEnabled() { return d->help; }

void QCommandLine::enableVersion(bool enable) { d->version = enable; }

bool QCommandLine::versionEnabled() { return d->version; }
...
void QCommandLine::addOption(
    const QChar& shortName, const QString& longName, const QString& descr, QCommandLine::Flags flags) {
...
void QCommandLine::addParam(const QString& name, const QString& descr, QCommandLine::Flags flags) {
    QCommandLineConfigEntry entry;
...
QString QCommandLine::version() {
    ...
    if (!QCoreApplication::organizationDomain().isEmpty() || !QCoreApplication::organizationName().isEmpty())
        v = v + QLatin1String(" - ") + QCoreApplication::organizationDomain() + QLatin1String(" ")
...
void QCommandLine::showHelp(bool quit, int returnCode) {
    ...
    if (quit) {
        // Can't call QApplication::exit() here, because we may be called before
        // app.exec()
        exit(returnCode);
    }
}
