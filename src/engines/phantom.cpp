...
void Phantom::init()
{
    ...
    // Line ~117: Original connect(m_page, SIGNAL(javaScriptConsoleMessageSent(QString)), SLOT(printConsoleMessage(QString)));
    // Line ~118: Original connect(m_page, SIGNAL(initialized()), SLOT(onInitialized()));
    // Updated connections:
...
bool Phantom::execute()
{
    ...
    if (m_config.scriptFile().isEmpty()) { // REPL mode requested
        ...
        if (m_config.debug()) {
            ...
            // Line ~211: Original Utils::loadJSForDebug(m_config.scriptFile(), m_scriptFileEnc, QDir::currentPath(), m_page->mainFrame(), m_config.remoteDebugAutorun())) {
            // Updated:
            ...
            // Line ~221: Original if (!Utils::injectJsInFrame(m_config.scriptFile(), m_scriptFileEnc, QDir::currentPath(), m_page->mainFrame(), true)) {
            // Updated:
...
bool Phantom::injectJs(const QString& jsFilePath)
{
    ...
    // Now delegates to WebPage, which uses IEngineBackend
    // Line ~370: Original return Utils::injectJsInFrame(pre + jsFilePath, Encoding::UTF8, libraryPath(), m_page->mainFrame());
    // Updated:
