...
void Phantom::init() {
    ...
    // Line ~117: Original connect(m_page, SIGNAL(javaScriptConsoleMessageSent(QString)),
    // SLOT(printConsoleMessage(QString))); Line ~118: Original connect(m_page, SIGNAL(initialized()),
    // SLOT(onInitialized())); Updated connections:
    ...
    // Line ~151: Original m_page->applySettings(m_defaultPageSettings);
    // This line remains the same but its implementation in WebPage is now different.
...
bool Phantom::execute() {
    ...
    if (m_config.scriptFile().isEmpty()) { // REPL mode requested
        ...
        // Line ~197: Original REPL::getInstance(m_page->mainFrame(), this);
        // Updated:
        ...
        if (m_config.debug()) {
            ...
            // Line ~207: Original m_config.setRemoteDebugPort(m_page->showInspector(m_config.remoteDebugPort()));
            // This line remains the same, but WebPage::showInspector() is now different.
            ...
            // Line ~211: Original Utils::loadJSForDebug(m_config.scriptFile(), m_scriptFileEnc, QDir::currentPath(),
            // m_page->mainFrame(), m_config.remoteDebugAutorun())) { Updated:
            ...
            // Line ~221: Original if (!Utils::injectJsInFrame(m_config.scriptFile(), m_scriptFileEnc,
            // QDir::currentPath(), m_page->mainFrame(), true)) { Updated:
...
QObject* Phantom::createWebPage() {
    ...
    // Line ~284: Original page->applySettings(m_defaultPageSettings);
    // This line remains the same, but its implementation in WebPage is now different.
    page->applySettings(m_defaultPageSettings);

    // Show web-inspector if in debug mode
    if (m_config.debug()) {
        // Line ~290: Original page->showInspector(m_config.remoteDebugPort());
        // This line remains the same, but WebPage::showInspector() is now different.
...
void Phantom::loadModule(const QString& moduleSource, const QString& filename) {
    ...
    // Line ~358: Original m_page->mainFrame()->evaluateJavaScript(scriptSource);
    // Updated:
...
bool Phantom::injectJs(const QString& jsFilePath) {
    ...
    // Now delegates to WebPage, which uses IEngineBackend
    // Line ~370: Original return Utils::injectJsInFrame(pre + jsFilePath, Encoding::UTF8, libraryPath(),
    // m_page->mainFrame()); Updated:
...
void Phantom::setProxy(
    ...
    if (m_page) {
        // Line ~390: Original m_page->setProxy(proxy.hostName() + ":" + QString::number(proxy.port()));
        // Updated: Now passes a URL string for setProxy
...
QString Phantom::proxy() {
    ...
    // Line ~404: Original return proxy.hostName() + ":" + QString::number(proxy.port());
    // Updated to return a full URL format:
...
void Phantom::onInitialized() {
    ...
    // Line ~446: Original m_page->mainFrame()->addToJavaScriptWindowObject("phantom", this);
    // Updated:
    if (m_page && m_page->engineBackend()) { // Ensure backend is available
        ...
        // Line ~450: Original m_page->mainFrame()->evaluateJavaScript(Utils::readResourceFileUtf8(":/bootstrap.js"));
        // Updated:
...
bool Phantom::setCookies(const QVariantList& cookies) {
    ...
    if (success && m_page) {
        // Line ~471: Original m_page->setCookies(cookies);
        // This line remains the same but its implementation in WebPage is now different.
...
QVariantList Phantom::cookies() const {
    ...
    // Line ~481: (Optional: Can change to) return m_page->cookies();
...
bool Phantom::addCookie(const QVariantMap& cookie) {
    ...
    if (success && m_page) {
        // Line ~488: Original m_page->addCookie(cookie);
        // This line remains the same but its implementation in WebPage is now different.
...
bool Phantom::deleteCookie(const QString& cookieName) {
    if (!cookieName.isEmpty()) {
        ...
        if (success && m_page) {
            // Line ~498: Original m_page->deleteCookie(cookieName);
            // This line remains the same but its implementation in WebPage is now different.
...
void Phantom::clearCookies() {
    m_defaultCookieJar->clearCookies();
    if (m_page) {
        // Line ~508: Original m_page->clearCookies();
        // This line remains the same but its implementation in WebPage is now different.
        m_page->clearCookies();
    }
}
...
void Phantom::doExit(int code) {
    ...
    while (i.hasPrevious()) {
        ...
        // Line ~527: Original page->mainFrame()->setUrl(QUrl(QStringLiteral("about:blank")));
        // Updated:
...
// Line ~546: Original (Missing in previous phantom.cpp but added as conceptual in WebPage)
