    ...
    , m_cachedFocusedFrameName("")
{
    ...
    connect(m_engineBackend, &IEngineBackend::javaScriptConsoleMessageSent, this, &WebPage::handleEngineJavaScriptConsoleMessageSent);
    ...
    connect(m_engineBackend, &IEngineBackend::javaScriptConfirmRequested, this, &WebPage::handleEngineJavaScriptConfirmRequested);
    connect(m_engineBackend, &IEngineBackend::javaScriptPromptRequested, this, &WebPage::handleEngineJavaScriptPromptRequested);
    connect(m_engineBackend, &IEngineBackend::javascriptInterruptRequested, this, &WebPage::handleEngineJavascriptInterruptRequested);
...
bool WebPage::render(const QString& fileName, const QVariantMap& option) {
    ...
    QString format = option.value(PAGE_SETTINGS_FORMAT, "png").toString().toLower();
    bool onlyViewport = option.value(PAGE_SETTINGS_ONLY_VIEWPORT, false).toBool();
    QPoint scrollPosition = m_engineBackend->scrollPosition();

    QRect clipRect;
    if (option.contains(PAGE_SETTINGS_CLIP_RECT)) {
        QVariantMap clipMap = option.value(PAGE_SETTINGS_CLIP_RECT).toMap();
        clipRect = QRect(clipMap.value("left").toInt(), clipMap.value("top").toInt(),
                         clipMap.value("width").toInt(), clipMap.value("height").toInt());
    } else {
...
QString WebPage::renderBase64(const QByteArray& format) {
    ...
    QPoint scrollPosition = m_engineBackend->scrollPosition();
    bool onlyViewport = m_engineBackend->viewportSize().isValid(); // This logic might need refinement if PAGE_SETTINGS_ONLY_VIEWPORT is used differently
...
void WebPage::setClipRect(const QVariantMap& size) {
    m_cachedClipRect = QRect(size.value("left").toInt(), size.value("top").toInt(),
                             size.value("width").toInt(), size.value("height").toInt());
...
void WebPage::applySettings(const QVariantMap& def) {
    if (def.contains(PAGE_SETTINGS_USER_AGENT)) setUserAgent(def[PAGE_SETTINGS_USER_AGENT].toString());
    if (def.contains(PAGE_SETTINGS_VIEWPORT_SIZE)) setViewportSize(def[PAGE_SETTINGS_VIEWPORT_SIZE].toMap());
    if (def.contains(PAGE_SETTINGS_CLIP_RECT)) setClipRect(def[PAGE_SETTINGS_CLIP_RECT].toMap());
    if (def.contains(PAGE_SETTINGS_SCROLL_POSITION)) setScrollPosition(def[PAGE_SETTINGS_SCROLL_POSITION].toMap());
    if (def.contains(PAGE_SETTINGS_ZOOM_FACTOR)) setZoomFactor(def[PAGE_SETTINGS_ZOOM_FACTOR].toReal());
    if (def.contains(PAGE_SETTINGS_CUSTOM_HEADERS)) setCustomHeaders(def[PAGE_SETTINGS_CUSTOM_HEADERS].toMap());
    if (def.contains(PAGE_SETTINGS_NAVIGATION_LOCKED)) setNavigationLocked(def[PAGE_SETTINGS_NAVIGATION_LOCKED].toBool());

    if (def.contains(PAGE_SETTINGS_DISK_CACHE_ENABLED)) m_engineBackend->setDiskCacheEnabled(def[PAGE_SETTINGS_DISK_CACHE_ENABLED].toBool());
    if (def.contains(PAGE_SETTINGS_MAX_DISK_CACHE_SIZE)) m_engineBackend->setMaxDiskCacheSize(def[PAGE_SETTINGS_MAX_DISK_CACHE_SIZE].toInt());
    if (def.contains(PAGE_SETTINGS_DISK_CACHE_PATH)) m_engineBackend->setDiskCachePath(def[PAGE_SETTINGS_DISK_CACHE_PATH].toString());
    if (def.contains(PAGE_SETTINGS_IGNORE_SSL_ERRORS)) m_engineBackend->setIgnoreSslErrors(def[PAGE_SETTINGS_IGNORE_SSL_ERRORS].toBool());
    if (def.contains(PAGE_SETTINGS_SSL_PROTOCOL)) m_engineBackend->setSslProtocol(def[PAGE_SETTINGS_SSL_PROTOCOL].toString());
    if (def.contains(PAGE_SETTINGS_SSL_CIPHERS)) m_engineBackend->setSslCiphers(def[PAGE_SETTINGS_SSL_CIPHERS].toString());
    if (def.contains(PAGE_SETTINGS_SSL_CERTIFICATES_PATH)) m_engineBackend->setSslCertificatesPath(def[PAGE_SETTINGS_SSL_CERTIFICATES_PATH].toString());
    if (def.contains(PAGE_SETTINGS_SSL_CLIENT_CERTIFICATE_FILE)) m_engineBackend->setSslClientCertificateFile(def[PAGE_SETTINGS_SSL_CLIENT_CERTIFICATE_FILE].toString());
    if (def.contains(PAGE_SETTINGS_SSL_CLIENT_KEY_FILE)) m_engineBackend->setSslClientKeyFile(def[PAGE_SETTINGS_SSL_CLIENT_KEY_FILE].toString());
    if (def.contains(PAGE_SETTINGS_SSL_CLIENT_KEY_PASSPHRASE)) m_engineBackend->setSslClientKeyPassphrase(def[PAGE_SETTINGS_SSL_CLIENT_KEY_PASSPHRASE].toByteArray());
    if (def.contains(PAGE_SETTINGS_RESOURCE_TIMEOUT)) m_engineBackend->setResourceTimeout(def[PAGE_SETTINGS_RESOURCE_TIMEOUT].toInt());
    if (def.contains(PAGE_SETTINGS_MAX_AUTH_ATTEMPTS)) m_engineBackend->setMaxAuthAttempts(def[PAGE_SETTINGS_MAX_AUTH_ATTEMPTS].toInt());

    if (def.contains(PAGE_SETTINGS_OFFLINE_STORAGE_PATH)) m_cachedOfflineStoragePath = def[PAGE_SETTINGS_OFFLINE_STORAGE_PATH].toString();
    if (def.contains(PAGE_SETTINGS_OFFLINE_STORAGE_QUOTA)) m_cachedOfflineStorageQuota = def[PAGE_SETTINGS_OFFLINE_STORAGE_QUOTA].toInt();
    if (def.contains(PAGE_SETTINGS_LOCAL_STORAGE_PATH)) m_cachedLocalStoragePath = def[PAGE_SETTINGS_LOCAL_STORAGE_PATH].toString();
    if (def.contains(PAGE_SETTINGS_LOCAL_STORAGE_QUOTA)) m_cachedLocalStorageQuota = def[PAGE_SETTINGS_LOCAL_STORAGE_QUOTA].toInt();
...
QNetworkProxy WebPage::proxy() const {
...
bool WebPage::setCookies(const QVariantList& cookies) {
    return m_engineBackend->setCookies(cookies);
}
QVariantList WebPage::cookies() const {
    return m_engineBackend->cookies();
}
bool WebPage::addCookie(const QVariantMap& cookie) {
    return m_engineBackend->addCookie(cookie);
}
bool WebPage::deleteCookie(const QString& cookieName) {
    return m_engineBackend->deleteCookie(cookieName);
}
void WebPage::clearCookies() {
    m_engineBackend->clearCookies();
}
...
void WebPage::clearMemoryCache() {
    m_engineBackend->clearMemoryCache();
}
...
int WebPage::showInspector(const int port) {
    return m_engineBackend->showInspector(port);
}
...
bool WebPage::javaScriptPrompt(const QString& msg, const QString& defaultValue, QString* result) {
    qDebug() << "WebPage: JS Prompt requested: " << msg << "Default:" << defaultValue;
    QString promptResult;
    bool acceptedByHandler = false;

    // Simulate user input for prompt. For a real UI, this would involve a dialog.
    // For now, let's just use the defaultValue and accept it.
    promptResult = defaultValue;
    acceptedByHandler = true;

    if (result) {
        *result = promptResult;
    }
    // The `accepted` output parameter of IEngineBackend::javaScriptPromptRequested
    // is set in handleEngineJavaScriptPromptRequested using the return value of this function.
    return acceptedByHandler;
}
...
void WebPage::handleEngineLoadingProgress(int progress) {
...
void WebPage::handleEngineNavigationRequested(const QUrl& url, const QString& navigationType, bool isMainFrame, bool navigationLocked) {
    qDebug() << "WebPage: Navigation requested to:" << url << "Type:" << navigationType << "MainFrame:" << isMainFrame << "Locked:" << navigationLocked;
...
void WebPage::handleEngineJavaScriptPromptRequested(const QString& message, const QString& defaultValue, QString* result, bool* accepted) {
    ...
    if (accepted) { // Pass the accepted status back to the backend
...
void WebPage::handleEngineResourceReceived(const QVariantMap& responseData) {
    emit resourceReceived(responseData);
}
void WebPage::handleEngineResourceError(const QVariantMap& errorData) {
    emit resourceError(errorData);
}
void WebPage::handleEngineResourceTimeout(const QVariantMap& errorData) {
